...
import static com.mk.library.Manager.utils.ErrorCode.ERROR_FILE_OUT_LIMIT;
...
public class DownloadBlockTask extends Task {
    ...
    public DownloadBlockTask(DownloadRequest downloadRequest, int blockId, DownloadConnection connection) {
        ...
        if (connection == null) {
            this.connection = DownloadFactory.getService(IDownloadConfigService.class).getDownloadConnectionFactory().create(downloadRequest.getHttpRequestBuilder());
        } else {
